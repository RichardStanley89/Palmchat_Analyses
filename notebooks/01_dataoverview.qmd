...
hacienda_edges <- edges_for_network %>% filter(Site == "Hacienda") %>% ungroup() %>% select(-Site)
...
E(hacienda_network)$weight

```

```{r}

# Create network object with statnet
...
# Add edge weights as an attribute
set.edge.attribute(hacienda_network, "weight", hacienda_edges$weight)

# Now use statnet functions (different names):
cat("Number of birds:", network.size(hacienda_network), "\n")
cat("Number of unique dyads:", network.edgecount(hacienda_network), "\n")
cat("Network density:", network.density(hacienda_network), "\n")
cat("Average degree:", mean(sna::degree(hacienda_network, cmode = "indegree")), "\n")
