...
```{r}
# Create vectors for In1, and 2
# match and keep only distinct ones
# Identify characters that are not valid (-), or missing

# The correct 
# They should all be following 4 letters

band_ids <- unique(c(clean_edges$Individual1, clean_edges$Individual2))


tibble(band_ids = band_ids) %>% 
  mutate(band1 = str_trim(band_ids)) %>% 
  # mutate(match_ = match(band_ids, band1)) # WANT TO get a true false
  filter(str_detect(band_ids, "-"))

# we need to identify which/match which ones they could be.
# match by placement

#W-WM this one is a true id because they ran out of bands

# isolate the ones that have only 3 or 2
```
...
clean_edges$dyad_id <- apply(clean_edges[,c("Individual1","Individual2")], 1, FUN= function(x) paste(sort(x), collapse="_"))
