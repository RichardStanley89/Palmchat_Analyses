...
plot(comm_louvain, hacienda_network,
     ...
     main = "Site 1 Social Network")

# Or with more control:
library(ggraph)
library(tidygraph)

net_tidy <- as_tbl_graph(hacienda_network) %>%
  activate(nodes) %>%
  mutate(community = as.factor(groups))

ggraph(net_tidy, layout = "fr") +  # Fruchterman-Reingold layout
  geom_edge_link(aes(width = weight), alpha = 0.3, color = "gray50") +
  geom_node_point(aes(color = community), size = 3) +
  scale_edge_width(range = c(0.2, 2)) +
  theme_graph() +
  labs(title = "Palmchat Social Network - Site 1",
       subtitle = paste("Modularity =", round(modularity(comm_louvain), 3)))
```
