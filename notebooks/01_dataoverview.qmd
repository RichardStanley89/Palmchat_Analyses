...
attribute_ids <- hacienda_attributes_clean$colors
missing_birds <- setdiff(hacienda_ids, attribute_ids)
# Why do we have missing birds from the attribute?

hacienda_edges %>% 
  filter(ID1 %in% missing_birds | ID2 %in% missing_birds)
# These are just once, there must be an observation error. 



# This needs to be cleaned but I'm not sure how to decide
# now it also makes me wonder if making the analysis by site is ideal or not

hacienda_ids %in% hacienda_attributes$colors
hacienda_ids[46]
# This one was observed but is missing from the captures
hacienda_attributes %>% filter(colors == "GSSM")
```
