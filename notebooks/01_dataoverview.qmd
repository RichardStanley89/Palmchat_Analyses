...
clean_edges_with_dyad_id <- clean_edges_no_dash %>% 
  ...
  # combine for id
  mutate(dyad_id = paste(sorted_Individual1, sorted_Individual2, sep = "-"), .after = Individual2) %>%
...
I want to see how many unique values we have, and how many times we saw them, and where. We have ~600 observations, and ~ distinct dyads

```{r}
clean_edges_with_dyad_id %>% 
  filter(dyad_id == " YGO-NGBM")
  dplyr::count(dyad_id)
```
...
clean_edges_no_dash %>% 
  # filter(Individual2 %in% band_ids_3)
  filter(Individual1 %in% band_ids_3)
```
